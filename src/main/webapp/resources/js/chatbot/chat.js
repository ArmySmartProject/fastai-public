...
$(document).ready(function () {
	...
	chatBotUiCtrl = (function(){
		...
		// 채팅 박스
		var chatBox = $(".callView_cont .lst_talk");
		...
		var swiperElem = $('<div class="swiper-slide">'+
								'<a class="swiper_item" href="#" target="_self">'+
					                '<span class="item_img"><img src="resources/images/sample/sample_swiper01.jpg" alt="이미지 명"></span>'+
		...
		var openChat = function(elem) {
			...
			$("#selectRoomId").val(room.roomId);
			$("#selectHost").val(room.csService);
  ...
  function writeChatList(rooms, option = 'av_rooms') {
  	var consultStatus = $('.consultStatus button.active').val();
  	if (consultStatus == '01') {
			if (rooms.length === 0) {
			...
			if (room['available']) {
				var host = room.csService;
				if (!availableRoomInfo.has(host)) {
					availableRoomInfo.set(host, 1);
				} else {
					availableRoomInfo.set(host, availableRoomInfo.get(host) + 1);
				}
			}
		});

			for (var [key, value] of availableRoomInfo) {
				notifyAvailableRooms(key, value);
			}
  ...
  writeMessage = function(roomId, talker, talkObj) {
	...
	notifyAvailableRooms = function(csService, count) {
		var serviceNm = getChatbotNm(csService);
  ...
  function getInputMsg() {
	...
	$('textarea[name="user_message"]').focus(function() {
		socketClient.enteringEvent();
	});

	$('textarea[name="user_message"]').blur(function() {
		socketClient.bluringEvent();
	});
  ...
  $('#btn_chat').on('click', function () {
    sendMessage();
		$('textarea[name="user_message"]').blur();
  });
  ...
  if (typeof userOption !== "undefined" && userOption === 'gcs') {
        ...
        socketClient.joinRoom(curRoomId).then(data => {
          // message 입력창 활성화
        	...
        	$('#btn_intervention').addClass('hide');
	 		...
	 		//상담메모 및 저장버튼 활성화
	 		$("#consultTxt").attr("disabled", false);
			$("#work_break").attr("disabled", false);
        })
        .catch(function (err) {
    ...
    $('#btn_call_end').click(function () {
      ...
      if (curRoomId) {
        ...
        $('textarea[name="user_message"]').attr('disabled',true);
        // 대화창 clear
				$('.callView_cont .lst_talk').html('');
				// 챗봇, 세션 정보 clear
				$('.chatbot_info_table td.cs_service').html('');
				$('.chatbot_info_table td.session_id').html('');
				$('.chatbot_info_table td.user_id').html('');

				//챗봇 상담메모 및 저장 버튼 disabled
				$("#consultTxt").attr("disabled", true);
				$("#consultTxt").val("");
				$("#work_break").attr("disabled", true);
        ...
        carousel_div +=
            '<div class="swiper-slide"> \
                <a class="swiper_item intent" href="#" target="_self" data-intent="'+ intent.intent +'" data-display="'+ carousel_ele.display +'">\
                    <span class="item_img"><img src="' + intent.displayUrl + '" onError="this.src=\'${pageContext.request.contextPath}/resources/images/redtie.jpg\'"></span> \
