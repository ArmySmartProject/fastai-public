        ...
        <div id="contents">
            ...
            <div class="content">
                ...
                <div class="srchArea">
                    ...
                    <input type="hidden" id="channelArr" />
                    ...
                    <table class="tbl_line_view" summary="검색일시/총응대시간/제외여부/음성봇명/상담유형/회선번호/상담사/고객명/통화시간/봇,상담사 응대시간/대화이력으로 구성됨">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="iptBox">
                                    <input type="number" class="ipt_txt" id="minChatCnt" autocomplete="off">
                                </div>
                            </td>
                            <th>최대 발화수</th>
                            <td>
                                <div class="iptBox">
                                    <input type="number" class="ipt_txt" id="maxChatCnt" autocomplete="off">
                                </div>
                            </td>
                            <input type="hidden" class="ipt_txt" id="totalChatCnt" autocomplete="off">
                            <input type="hidden" class="ipt_txt" id="userFeedback" autocomplete="off">
                        </tr>
                        <tr>
                            <th>유입채널</th>
                            <td id="multiple3">
                                <select id="channel" class="select" multiple="multiple3">
                                    <option>테스트</option>
                                </select>
                            </td>
                            <th>상담자명</th>
...
// 		 	'고객명',
            ...
            colModel:[
                ...
                {name:'category', width: 150, index:'category', align:'center', sortable: false},
                {name:'userFeedback', width: 150, index:'userFeedback', align:'center', sortable: false},
...
// 		toDatePicker.minuteStep = 30;
    ...
    function getChannelList() {
        ...
        $.ajax({
            url : "${pageContext.request.contextPath}/getChannelList",
            data : JSON.stringify(obj),
            method : 'POST',
            contentType : "application/json; charset=utf-8",
            beforeSend : function(xhr) {
            ...
            $('#multiple3').empty();
            var innerHTML = "";
            innerHTML += '<select id="channel" class="select" multiple="multiple3" onchange="changeChannel(this.value);">';
            if(result != null && result.length > 0){
                for(var i = 0; i < result.length; i++){
                    innerHTML += '<option value="' + result[i] + '"selected>' + result[i] + '</option>';
                    channelArr.push(result[i]);
                }
            }
            innerHTML += '</select>'
            $("#channelArr").val(channelArr);
            $("#multiple3").append(innerHTML);
            $('#channel').multiselect({
                includeSelectAllOption: true,
            });
            ...
            console.log("ajax connection error: getChannelList");
        });
    }
    ...
    function changeCategory(){
        var category = "";
        if($("#category").val() != "" && $("#category").val() != null && $("#category option:selected").length > 0) {
            $.each($("#category").val(), function(i, v){
                if( i == 0){
                    category = v;
                }else if(i > 0){
                    category = category + "," + v;
    ...
    function changeChannel(){
        var channel = "";
        if($("#channel").val() != "" && $("#channel").val() != null && $("#channel option:selected").length > 0) {
            $.each($("#channel").val(), function(i, v){
                if( i == 0){
                    channel = v;
                }else if(i > 0){
                    channel = channel + "," + v;
    ...
    function gridSearch(currentPage, excelYn) {
        ...
        obj.minChatCnt = $("#minChatCnt").val();
        obj.maxChatCnt = $("#maxChatCnt").val();
        obj.category = $("#category").val();
        obj.userFeedback = $("#userFeedback").val();
        obj.categoryArr = $("#categoryArr").val();
        obj.channelArr = $("#channelArr").val();
