...
public class VocAnalyPagingVO {
    ...
    private int pageNum;                //웹 페이지 하단 한번에 보여지는 페이지의 개수

    private int startRowNumCurrentPage; 	//리스트 페이지의 No(단순 증가 번호).


    public VocAnalyPagingVO() {
        this(10,5);
    }

    public VocAnalyPagingVO(int initVal, int pageNum) {
    ...
    public void setCurrentPage(int currentPage) {
        ...
        pageRangeEnd = (int)Math.ceil( (double)currentPage / (double)pageNum ) * pageNum;

        //현제 페이지로 웹 페이지 하단에 있는 페이지 범위 시작 계산.
        pageRangeStart = pageRangeEnd - pageNum + 1;
        if( pageRangeStart <= 0) {
            pageRangeStart = 1;
        }
